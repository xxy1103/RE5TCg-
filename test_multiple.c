...
int create_dns_query(char* buffer, const char* hostname, unsigned short qtype) {
    ...
    // DNS头部
    *((unsigned short*)(buffer + offset)) = htons(0x5678); // 不同的ID
    offset += 2;
...
void test_query(const char* hostname, unsigned short qtype, const char* type_name) {
    WSADATA wsaData;
    SOCKET sock;
    ...
    // 创建DNS查询
    query_len = create_dns_query(query_buffer, hostname, qtype);
    ...
    if (ancount > 0) {
        printf("SUCCESS: Received %d answer(s)\n", ancount);
    } else {
        printf("INFO: No answers in response\n");
    }
...
int main() {
    ...
    // 测试A记录查询
    test_query("google.com", 1, "A");
    
    // 等待一秒
    Sleep(1000);
    
    // 测试AAAA记录查询
    test_query("google.com", 28, "AAAA");
    
    // 等待一秒
    Sleep(1000);
    
    // 测试另一个域名
    test_query("github.com", 1, "A");
